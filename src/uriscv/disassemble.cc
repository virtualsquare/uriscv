	...
	if (InvalidRegInstr(instr))
		...
		case SFN_MULTU:
			sprintf(strbuf, "%s\t$%s, $%s",
			        regIName[FUNC3(instr)], regName[RS1(instr)], regName[RS2(instr)]);
			break;

		case SFN_JALR:
			sprintf(strbuf, "%s\t$%s, $%s",
			        regIName[FUNC3(instr)], regName[RD(instr)], regName[RS1(instr)]);
			break;
	...
	case LUI:
		if (!RS1(instr))
			sprintf(strbuf, "%s\t$%s, 0x%.4lX",
			        iName[OPCODE(instr)], regName[RS2(instr)], ZEXTIMM(instr));
		else
...
HIDDEN void strLSInstr(Word instr)
{
	...
	case SWR:
		sprintf(strbuf, "%s\t$%s, %+ld($%s)",
		        iName[OPCODE(instr)], regName[RS2(instr)], (long int) SignExtImm(instr), regName[RS1(instr)]);
		break;
...
HIDDEN void strCopInstr(Word instr)
{
	...
	if (OPCODE(instr) == COP0SEL) {
		...
		case MFC0:
			...
			if (ValidCP0Reg(RD(instr), &cp0Num) && /* placeholder */ 0 && !FUNC3(instr))
			// if (ValidCP0Reg(RD(instr), &cp0Num) && !SHAMT(instr) && !FUNC3(instr))
				// istruction is valid
				sprintf(strbuf, "%s\t$%s, $%s", cp0IName[COPOPTYPE(instr)], regName[RS2(instr)], cp0RegName[cp0Num]);
			else
